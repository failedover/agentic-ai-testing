assistant_msg = {
    'role': 'system',
    'content': (
        'You are an AI assistant that has another AI model working to get you live data from a search '
        ...
        'that always impresses the user would generate.  You are going to be the agent providing this '
...
search_or_not_msg = (
    ...
    'If a user asks for any event that happened after December 31, 2024, you will need to use a search. '
    'If a user says the word today, this week, this month, or this year, you will need to use a search '
...
query_msg = (
    ...
    'AI has determined this prompt to the actual AI assistant, requiring a web search for more recent data. '
    'You must determine what the data the assistant needs from search and generate the best possible '
    'DuckDuckGo search query to find that data.  Do not respond with anything but a query that an expert human '
...
contains_data_msg = (
    ...
    '   PAGE_TEXT: "entire page text from the best search result based off of the search snippet." \n'
    ...
    'page text does indeed contain the reliable data for the AI assistant to use as a context to respond.  Respond '
